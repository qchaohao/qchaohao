...
### 命名规范
  ...
  - ***类*** 使用帕斯卡风格，私有类可用一个下划线开头；函数名小写用下划线隔开，私有函数在函数前加一个下划线'\_'；变量名小写，如有多个单词使用下划线'\_'隔开。
  
  - 将相关的类和顶级函数放在同一个模块里，最好一个类一个模块。

  
  - 文件最开始"#! /usr/bin/env python3"​，如无特殊情况必须加入"# -*- coding: utf-8 -*-"标识，"import"语句应该放在文件头部，置于模块说明及Docstring之后且置于全局变量之前。
  
  - 配置文件 小写，横线隔开, 配置内容小写，下划线隔开。
...
### 注释规范
  ...
  - 单行注释以 **（ # ）** 开头，多行注释/文档注释以三对引号括起来 **（ ''' ）** / **（ """ ）**。 
  - 所有的模块、类和函数都应该在头部写文档注释，这样通过对象的__doc__对象获取文档。
  - 文档注释不要中英文混用，内容简洁清晰，缩进错落有致，符号单独一行。
