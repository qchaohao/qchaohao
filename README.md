...
> - ***字体*** 使用 **星号<sup>!!!</sup>**，***分割线和列表*** 使用 **减号<sup>!!!</sup>**。
> - ***粗斜体的星号和括号*** 不相连使用，有需要的话用中文括号。
> - 内容首先要建立 ***无序列表***，区块引用用于上下文不同的内容区别。
> - ***表格和代码块*** 要单独写，不应该被其他标签包含。

### 标签规定
 
- **@@..>** 代表 ***NOTE*** 重要的提示或背景信息‌，**与众不同的**，使用粗体或粗斜体。
- **@@..!** 代表 ***FOOL*** 不能忘记或不能更改的，**不能出错的<sup>!!!</sup>**，使用粗体带三个叹号上标。
- **@@..?** 代表 ***TODO*** 待完成的或要待实现的，**未想明白的<sup>???</sup>**，使用粗体带三个问号上标。
- **@@..+** 代表 ***DOIT*** 已知问题或需要修复的，**加急去做的<sup>+++</sup>**，使用粗体带三个加号上标。
- **@@..=** 代表 ***DEAD*** 有争议或有重启的价值，~~历史信息的~~，只使用删除线。
- 标签颜色依次为 橙FF7F00，红FF0000，品FF00FF，青00FFFF，紫7F00FF。

### 思维导图

  - 一级标题不写内容，二级以上的标题最后一级才写内容，否则 ***不被 Markmap 识别***。
  - ***列表*** 不管多少级都可以写内容，每一行最好不超过一行。
  - ***非重要的*** 信息可以用 ***区块引用***，***不被 Markmap 识别***。
  - ***列表*** 和 ***表格或代码块*** 同一级不能同时存在，**会忽略列表<sup>!!!</sup>**。
...
- ***末尾换行***：使用2个 ***空格***，或标签 **\<br>** 前后紧跟内容。
- ***上标***：使用标签 \<sup>上标\</sup> 包围，**2<sup>10</sup>**。
- ***下标***：使用标签 \<sub>下标\</sub> 包围，**H<sub>2</sub>O**。
- ***空格***：标记后面一个空格，如果文本要空格使用 ***&nbsp;***。
- ***Tab***：标记后面一个Tab，如果文本要空格使用 ***&emsp;***。
- ***空白行***：将一行或多行文本进行分隔，创建段落。
...
- 语法：使用一个 ***减号*** **( - )**、加号( + ) 然后加上一个空格。
- 说明：理论上可以无限嵌套，下层标记符位于上方第一个文本字符的正下方。
> 举例：
> - 这是无序第一层  
> - 这是换行  
>    - 这是无序第二层 
